...
// 给定一条 event 的字段 + 当前 epoch ms → 派生 {days_until, next_at, milestone, age_label}。
function derive(e, nowMs) {
  ...
  let milestone = "";
  if (e.kind === "birthday") milestone = years > 0 ? "满 " + years + " 岁" : "出生";
  else if (e.kind === "anniversary") milestone = years > 0 ? "第 " + years + " 周年" : "";
  ...
  if (e.kind === "birthday" && recurring) {
    ...
    if (ageDays >= 0 && ageDays < 365) {
      age_label = ageDays < 100 ? "出生 " + ageDays + " 天" : Math.floor(ageDays / 30.44) + " 个月";
    }
  }

  return { days_until, next_at: nxt.mo + "月" + nxt.d + "日", milestone, age_label };
}
...
async function refresh(_args, ctx) {
  ...
  for (const rec of items) {
    const patch = derive(rec.data || rec, nowMs);
